...
void ColumnVector::WriteAdv(char *&ptr) const {
    ...
    // write variable part
    if (const auto data_t = data_type_->type(); data_t == kVarchar) {
        i32 heap_len = buffer_->var_buffer_mgr_->TotalSize();
        WriteBufAdv<i32>(ptr, heap_len);
        buffer_->var_buffer_mgr_->Write(ptr);
    } else if (data_t == kTensor or data_t == kTensorArray or data_t == kSparse) {
...
SharedPtr<ColumnVector> ColumnVector::ReadAdv(char *&ptr, i32 maxbytes) {
    ...
    if (const auto data_t = data_type->type(); data_t == kVarchar) {
        ...
        column_vector->buffer_->var_buffer_mgr_->Append(ptr, heap_len);
    } else if (data_t == kTensor or data_t == kTensorArray or data_t == kSparse) {
        i32 heap_len = ReadBufAdv<i32>(ptr);
...
void CopyVarchar(VarcharT &dst_ref, VarBuffer *dst_var_buffer, const VarcharT &src_ref, VarBuffer *src_var_buffer) {
