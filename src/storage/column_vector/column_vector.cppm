...
public:
    ...
    friend std::strong_ordering ThreeWayCompareReaderValue(const IteratorType &left, const IteratorType &right) {
        ...
        auto *left_data = left.var_buffer_mgr_->Get(left_value.vector1_.file_offset_, left_value.length_);
        auto *right_data = right.var_buffer_mgr_->Get(right_value.vector1_.file_offset_, right_value.length_);
        return std::strncmp(left_data, right_data, std::min(left_length, right_length)) <=> 0;
    }
    friend bool CheckReaderValueEquality(const IteratorType &left, const IteratorType &right) {
        ...
        auto *left_data = left.var_buffer_mgr_->Get(left_value.vector1_.file_offset_, left_length);
        auto *right_data = right.var_buffer_mgr_->Get(right_value.vector1_.file_offset_, right_length);
